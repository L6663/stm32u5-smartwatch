...
#define LCD_TX_LINES           1U
#define LCD_SPI_TIMEOUT_MS     1000U

/*
 * 320像素 × 8行 × 2字节 = 5120字节。
 * SPI保持8位模式，发送前把RGB565转换为ILI9341要求的高字节优先格式。
 */
static uint8_t lcd_tx_buffer[LCD_MAX_WIDTH * LCD_TX_LINES * 2U];

/* 仅用于调试，可在Watch窗口观察。 */
volatile uint32_t g_lcd_flush_count = 0U;
volatile uint32_t g_lcd_flush_error = 0U;
...
{
    uint8_t TempBufferD[2];

    TempBufferD[0] = (uint8_t)(DAT >> 8);
    TempBufferD[1] = (uint8_t)(DAT & 0xFFU);

    (void)HAL_SPI_Transmit(&hspi1,
                           TempBufferD,
                           2U,
                           LCD_SPI_TIMEOUT_MS);
}
...
void ILI9341_Clear(uint16_t color)
{
    ...
    uint32_t bufferPixels;
    uint32_t index;
    uint16_t transferPixels;
    uint8_t highByte;
    uint8_t lowByte;

    highByte = (uint8_t)(color >> 8);
    lowByte  = (uint8_t)(color & 0xFFU);

    bufferPixels = LCD_MAX_WIDTH * LCD_TX_LINES;

    for (index = 0U; index < bufferPixels; index++)
    {
        lcd_tx_buffer[index * 2U]      = highByte;
        lcd_tx_buffer[index * 2U + 1U] = lowByte;
    }

    ILI9341_SetArea(0U,
                    0U,
                    (uint16_t)(ILI9341dev.width - 1U),
                    (uint16_t)(ILI9341dev.height - 1U));
    ILI9341_WriteRAM_Prepare();

    totalPixels = (uint32_t)ILI9341dev.width *
                  (uint32_t)ILI9341dev.height;
    pixelsRemaining = totalPixels;

    while (pixelsRemaining > 0U)
    {
        transferPixels = (pixelsRemaining > bufferPixels)
                       ? (uint16_t)bufferPixels
                       : (uint16_t)pixelsRemaining;

        if (HAL_SPI_Transmit(&hspi1,
                             lcd_tx_buffer,
                             (uint16_t)(transferPixels * 2U),
                             LCD_SPI_TIMEOUT_MS) != HAL_OK)
        {
            g_lcd_flush_error = 3U;
            return;
        }

        pixelsRemaining -= transferPixels;
    }
}
/*
**********************************************************************
...
**********************************************************************
*/
void _HW_FillFrame(uint16_t sx,
                   uint16_t sy,
                   uint16_t ex,
                   uint16_t ey,
                   uint16_t *color)
{
    ...
    g_lcd_flush_error = 0U;

    if ((color == NULL) ||
        (ex < sx) ||
        (ey < sy))
    {
        g_lcd_flush_error = 1U;
        return;
    }
    ...
    /*
     * SPI始终保持8位模式。
     * 不在运行时修改DataSize，不使用DMA，避免此前黑屏问题。
     */
    ...
    {
        rowsThisTransfer = (rowsRemaining > LCD_TX_LINES)
                         ? LCD_TX_LINES
                         : rowsRemaining;

        pixelCount = (uint32_t)width *
                     (uint32_t)rowsThisTransfer;

        /*
         * STM32内存中的uint16_t为小端；
         * ILI9341串行RGB565要求高字节先发送。
         */
        for (index = 0U; index < pixelCount; index++)
        {
            pixel = source[index];

            lcd_tx_buffer[index * 2U] =
                (uint8_t)(pixel >> 8);

            lcd_tx_buffer[index * 2U + 1U] =
                (uint8_t)(pixel & 0xFFU);
        }

        status = HAL_SPI_Transmit(&hspi1,
                                  lcd_tx_buffer,
                                  (uint16_t)(pixelCount * 2U),
                                  LCD_SPI_TIMEOUT_MS);

        if (status != HAL_OK)
        {
            g_lcd_flush_error = 2U;
            return;
        }

        source += pixelCount;
        rowsRemaining =
            (uint16_t)(rowsRemaining - rowsThisTransfer);
    }
